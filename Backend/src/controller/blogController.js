...
export const getBlogs = async (req, res) => {
  try {
    ...
    res.json(blogs.map((blog) => toDTO(blog)));
  } catch (err) {
    res.status(500).json({
      message: err.message,
    });
...
export const createBlog = async (req, res) => {
  ...
  try {
    ...
    if (req.file?.buffer) {
      image = await uploadImageBuffer(req.file.buffer, "vehicle-rental/blogs");
    }
    ...
    res.status(201).json(toDTO(blog));
  } catch (err) {
    console.log(err);
...
export const updateBlog = async (req, res) => {
  try {
    ...
    if (req.file?.buffer) {
      updates.image = await uploadImageBuffer(
        req.file.buffer,
        "vehicle-rental/blogs",
      );
    }

    const blog = await Blog.update(req.params.id, updates);
