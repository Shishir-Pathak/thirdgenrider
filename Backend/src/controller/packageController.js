...
function toDTO(o) {
  ...
  return {
    ...
    itinerary: o.itinerary || o.itinerary || [],

    tripHighlights: o.tripHighlights || o.trip_highlights || [],

    inclusions: o.inclusions || o.inclusions || [],

    packageExperience: o.package_experience || "",
...
function parseItinerary(raw) {
  ...
  for (const item of parsed) {
    const dayNumber = item?.dayNumber?.trim() || "";
...
function parseStringList(raw, label) {
  ...
  return {
    items: parsed.map((i) => i.trim()).filter(Boolean),
  };
}
...
function validatePackageBody(body, res) {
  ...
  const packageExperience = body.packageExperience?.trim() || "";
  ...
  if (itineraryResult.error) {
    res.status(400).json({
      message: itineraryResult.error,
    });

    return null;
  }

  const highlightsResult = parseStringList(
    body.tripHighlights,
    "Trip highlights",
  );
  ...
  if (highlightsResult.error) {
    res.status(400).json({
      message: highlightsResult.error,
    });
  ...
  if (inclusionsResult.error) {
    res.status(400).json({
      message: inclusionsResult.error,
    });
...
export const getPackages = async (req, res) => {
  try {
    ...
    res.status(200).json(packages.map((p) => toDTO(p)));
  } catch (err) {
    res.status(500).json({
      message: err.message,
    });
...
export const deletePackage = async (req, res) => {
  try {
    const deleted = await Package.delete(req.params.id);
...
export const updatePackage = async (req, res) => {
  try {
    ...
    const updated = await Package.update(id, updatedData);
